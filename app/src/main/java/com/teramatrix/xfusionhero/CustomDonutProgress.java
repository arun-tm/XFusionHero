...
public class CustomDonutProgress extends View {
    ...
    //custom
    private Paint reportOKPaint;
    private Paint reportNGPaint;
    private Paint reportNotSubmittedPaint;
    ...
    //custom
    private int reportOkColor;
    private int reportNGColor;
    private int reportNotSubmittedColor;
    ...
    private int report_ok_start_angle = 0;
    private int report_ok_end_angle = 0;

    private int report_ng_start_angle = 0;
    private int report_ng_end_angle = 0;

    private int report_not_sub_start_angle = 0;
    private int report_not_sub_end_angle = 0;
    ...
    protected void initPainters() {
        ...
        //custom
        reportOKPaint = new Paint();
        reportOKPaint.setColor(reportOkColor);
        reportOKPaint.setStyle(Paint.Style.STROKE);
        reportOKPaint.setAntiAlias(true);
        reportOKPaint.setStrokeWidth(finishedStrokeWidth);

        reportNGPaint = new Paint();
        reportNGPaint.setColor(reportNGColor);
        reportNGPaint.setStyle(Paint.Style.STROKE);
        reportNGPaint.setAntiAlias(true);
        reportNGPaint.setStrokeWidth(finishedStrokeWidth);

        reportNotSubmittedPaint = new Paint();
        reportNotSubmittedPaint.setColor(reportNotSubmittedColor);
        reportNotSubmittedPaint.setStyle(Paint.Style.STROKE);
        reportNotSubmittedPaint.setAntiAlias(true);
        reportNotSubmittedPaint.setStrokeWidth(finishedStrokeWidth);
        ...
        defaultPaint.setStrokeWidth(finishedStrokeWidth);
    ...
    protected void initByAttributes(TypedArray attributes) {
        ...
        reportOkColor = context.getResources().getColor(R.color.color_nam_menu_dark_gray);
        reportNGColor = context.getResources().getColor(R.color.color_hero_red);
        reportNotSubmittedColor = Color.LTGRAY;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        //custom
        if (report_not_sub_start_angle == 0 && report_not_sub_end_angle == 0) {
            //If No Final Reports , paint donut to default gray color to apear hollow
            canvas.drawArc(finishedOuterRect, 0, 360, false, reportNotSubmittedPaint);
        } else {
            canvas.drawArc(finishedOuterRect, 0, 360, false, defaultPaint);
            canvas.drawArc(finishedOuterRect, report_ok_start_angle + 2, report_ok_end_angle - 2, false, reportOKPaint);
            canvas.drawArc(finishedOuterRect, report_ng_start_angle + 2, report_ng_end_angle - 2, false, reportNGPaint);
            canvas.drawArc(finishedOuterRect, report_not_sub_start_angle + 2, report_not_sub_end_angle - 2, false, reportNotSubmittedPaint);
        }
        ...
        if (!TextUtils.isEmpty(getInnerBottomText())) {
            innerBottomTextPaint.setTextSize(innerBottomTextSize);
            float bottomTextBaseline = getHeight() - innerBottomTextHeight - (textPaint.descent() + textPaint.ascent()) / 2 + 15;
    ...
    public void setDonutProgress(int max_value, int val_1, int val_2, int val_3) {
        try {
            report_ok_start_angle = 0;
            report_ok_end_angle = (val_1 * 360 / max_value);

            report_ng_start_angle = report_ok_end_angle;
            report_ng_end_angle = (val_2 * 360 / max_value);

            report_not_sub_start_angle = report_ng_start_angle + report_ng_end_angle;
            report_not_sub_end_angle = (val_3 * 360 / max_value);
        } catch (Exception e) {
            report_ok_start_angle = 0;
            report_ok_end_angle = 0;

            report_ng_start_angle = 0;
            report_ng_end_angle = 0;

            report_not_sub_start_angle = 0;
            report_not_sub_end_angle = 0;
            e.printStackTrace();
        }
